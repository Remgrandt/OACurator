...
impl Catalog {
    ...
    pub fn create_collection_with_provider_ids(
        ...
        let manifest_path = manifest_path_from_input(
            &expand_user_path(&manifest_path.to_string_lossy()),
            &name,
            "oacollection",
            "Untitled Collection",
        );
        require_absolute_manifest_path(&manifest_path, "Collection")?;
    ...
    pub fn create_gallery_with_provider_ids(
        ...
        require_absolute_manifest_path(&manifest_path, "Gallery")?;
        let path_string = manifest_path.to_string_lossy().to_string();
        let id = {
            let conn = self.lock()?;
            conn.execute(
...
fn require_absolute_manifest_path(path: &Path, label: &str) -> Result<()> {
    if path.is_absolute() {
        return Ok(());
    }
    Err(AppError::Message(format!(
        "{label} path must be an absolute folder or manifest path"
    )))
}
